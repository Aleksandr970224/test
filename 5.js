...
let newStrings = [strings[0]];

let unique = (arr) => {
  for (let i = 0; i < arr.length; i++) {
      if (!newStrings.find(item => item === arr[i])) {  //создаем функцию, с параметром arr, далее объявляем цикл, чтобы обойти исходный массив. Затем условие: перебираем новый массив методом .find(), и если новый массив содержит перебираемый элемент исходного массива - то true, НО мы меняем на false и тогда элемент в новый массив не пушиться. А если условие false, то мы меняем на true - и выполняется тело условия
...
// let unique = (arr) => {
//   return arr.filter((arrItem, index) => arr.indexOf(arrItem) === index)
// };
